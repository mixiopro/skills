...
7. **Resolve Before Finalizing**: Resolve every authored `#` character, location, or prop token — `resolve_mention` per distinct token, or diff them against `mentionableLooks`. Because an unresolved mention fails soft and `upsert_screenplay` reports no mention diagnostics, a body full of literal `#` text writes exactly like one where every token bound. Register or enrich whatever is missing, obtain approval for new references/looks, put the exact token back in the body, and re-upsert: the loop is [`mixio-pipeline/references/screenplay-reference-loop.md`](../../mixio-pipeline/references/screenplay-reference-loop.md).
