...
7. **Resolve Before Finalizing**: Resolve every `#` token you wrote — `resolve_mention` per distinct token, or diff them against `mentionableLooks`. Because an unresolved mention fails soft and `upsert_screenplay` reports no mention diagnostics, a body full of literal `#` text writes exactly like one where every token bound. Register or render whatever is missing, put the real token back in the body, and re-upsert: the loop is [`mixio-pipeline/references/screenplay-reference-loop.md`](../../mixio-pipeline/references/screenplay-reference-loop.md).
