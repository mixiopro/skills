...
- MULTI_KF shots use `keyframe-sequence` use case with `keyframe_count` parameter (4–12). The keyframes themselves are generated as images first, then the sequence job stitches them. Budget 1 image job per keyframe + 1 video job per shot.
