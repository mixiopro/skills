...
const reservedExtensionProperties = new Set(['id', 'object', 'type', 'status', 'ok', 'terminal', 'evaluation', 'model', 'output', 'contract-version', 'catalog-version', 'verdict', 'decision', 'passed', 'score', 'threshold', 'confidence', 'profile', 'metrics', 'findings', 'evidence', 'coverage', 'resolved-plan', 'skill-outputs', 'extensions', 'input-alias', 'source-url', 'required-floor', 'finding-ids', 'evidence-ids', 'run-id'])
...
const completed = {
  ...
  terminal: true,
  threshold: 0.8,
  evaluation: { id: 'response-example', status: 'completed', decision: 'pass' }
}
...
const expectedTransitionIds = transitions.map(candidate => candidate.id)
const strictGate = (receipt, review) => {
  const transitionsComplete = Array.isArray(review.transitions) &&
    review.transitions.length === expectedTransitionIds.length &&
    review.transitions.every(candidate => (
      typeof candidate['transition-id'] === 'string' &&
      expectedTransitionIds.includes(candidate['transition-id']) &&
      candidate['transition-status'] === 'pass' &&
      Array.isArray(candidate['transition-findings']) &&
      candidate['transition-findings'].every(finding => (
        typeof finding['finding-id'] === 'string' &&
        finding['transition-id'] === candidate['transition-id'] &&
        typeof finding.dimension === 'string' &&
        finding.blocking === false &&
        finding.classification === 'intended_change'
      ))
    ))
  const worstTransition = review['worst-transition']
  const findings = [
    ...(Array.isArray(review['blocking-findings']) ? review['blocking-findings'] : []),
    ...(Array.isArray(review.transitions) ? review.transitions.flatMap(candidate => candidate['transition-findings'] || []) : [])
  ]
  return (
    receipt.ok === true &&
    receipt.status === 'completed' &&
    receipt.terminal === true &&
    typeof receipt.threshold === 'number' && receipt.threshold >= 0 && receipt.threshold <= 1 &&
    receipt.evaluation?.decision === 'pass' &&
    review['review-verdict'] === 'pass' &&
    typeof worstTransition?.['transition-id'] === 'string' &&
    expectedTransitionIds.includes(worstTransition['transition-id']) &&
    worstTransition['transition-status'] === 'pass' &&
    transitionsComplete &&
    Array.isArray(review['blocking-findings']) &&
...
const passingReview = {
  'review-verdict': 'pass',
  'worst-transition': { 'transition-id': transition.id, 'transition-status': 'pass' },
  'blocking-findings': [],
  transitions: [{ 'transition-id': transition.id, 'transition-status': 'pass', 'transition-findings': [] }]
}
if (!strictGate(completed, passingReview)) fail('complete passing review should pass')
if (strictGate({ ...completed, evaluation: { ...completed.evaluation, average: 0.99 } }, {
  ...passingReview,
  'blocking-findings': [{ 'finding-id': 'f-1', 'transition-id': transition.id, classification: 'accidental_drift', blocking: true }]
})) fail('blocking finding must win over average')
const declaredChange = [{ transitionId: transition.id, classification: 'intended_change', blocking: false }]
if (!strictGate(completed, { ...passingReview, transitions: [{ ...passingReview.transitions[0], 'transition-findings': [{ 'finding-id': 'f-1', 'transition-id': transition.id, dimension: 'camera', ...declaredChange[0] }] }] })) fail('declared change should not block')
if (strictGate(completed, { ...passingReview, 'blocking-findings': [{ 'finding-id': 'f-1', 'transition-id': transition.id, classification: 'intended_change', blocking: true }] })) fail('blocking intended change must still block')
if (strictGate(completed, { ...passingReview, 'blocking-findings': [{ 'finding-id': 'f-1', 'transition-id': transition.id, classification: 'unknown', blocking: false }] })) fail('unknown finding classification must fail closed')
if (strictGate({ ...completed, threshold: null }, passingReview)) fail('terminal run must report threshold')
if (strictGate({ ...completed, evaluation: { ...completed.evaluation, decision: undefined } }, passingReview)) fail('provider decision must be present')
if (strictGate(completed, { ...passingReview, transitions: [] })) fail('every transition must be localized')
if (strictGate(completed, { ...passingReview, transitions: [{ ...passingReview.transitions[0], 'transition-findings': [{ 'finding-id': 'f-1', 'transition-id': 'wrong-transition', dimension: 'camera', classification: 'intended_change', blocking: false }] }] })) fail('finding must bind to its transition')
