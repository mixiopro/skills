...
- Before authoring a screenplay, read `skills/mixio-episode/references/screenplay-grammar.md`, call `studio_list_references({ projectId, limit })`, and copy exact `mentionableLooks` values for `#name.variant[.view]`; never hand-build a mention. Preserve `~location.landmark[.placement]` locks and standalone `[Key: Value · Key: Value]` paragraphs through breakdown. `studio_upsert_screenplay` writes a draft only; approval remains a human Studio action. Authoring is a loop, not a single write: an unresolved `#` mention fails soft and `upsert_screenplay` reports nothing about it, so resolve every token with `studio_resolve_mention`, register or render what is missing, put the real token back in the body and re-upsert — `skills/mixio-pipeline/references/screenplay-reference-loop.md`.
