...
- Before authoring a screenplay, read `skills/mixio-episode/references/screenplay-grammar.md`, call `studio_list_references({ projectId, limit })`, and copy exact `mentionableLooks` values for `#name.variant[.view]`; never hand-build a mention. Preserve `~location.landmark[.placement]` locks and standalone `[Key: Value · Key: Value]` paragraphs through breakdown. `studio_upsert_screenplay` writes a draft only; approval remains a human Studio action. Use the pre-gate loop: extract character/location/prop candidates, resolve every authored `#` token, register/enrich and get approval for new references/looks, put the exact tokens back in the body, and re-upsert — `skills/mixio-pipeline/references/screenplay-reference-loop.md`.
